...
- `SymbolParser`: Language-specific symbol and metadata extraction, now using Tree-sitter for JS/TS/TSX, PHP, CSS, Python, Rust, Java, C#, and Swift where integrated, while retaining heuristics where coverage is still incomplete.
...
- **Relation Checks**: The sweep now includes basic relation-presence checks for languages where `Calls` / `Called By` should exist, while tolerating files that do not contain enough callable symbols to make that meaningful.
...
- Relation parity should also not be assumed yet; `Calls` and `Called By` payloads are still useful for regression checks, but remain incomplete and sometimes asymmetric.
